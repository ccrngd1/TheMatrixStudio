...
- **State/store:** reuse the Phase 0 SQLite event store as-is. No schema changes required for Phase 1.

**This is the one thing I want your explicit yes/no on** (§8.3 of the project spec left the web stack open for research). If you'd rather PreCog research alternatives (e.g. Svelte, or an HTMX/server-rendered lighter build) before committing, say so and I'll route that first. My recommendation is to proceed with the above — it's the lowest-surprise, most-hireable stack for a shipped tool.
...
- `POST /api/runs` — start a new run from a request body (same shape as the CLI request JSON: `topic`, `cast[]`, `config{max_messages, generate_avatars}`, optional `model`). Returns `run_id` immediately; the simulation runs in a background task. Never blocks the HTTP response on completion.
- `GET /api/runs` — list past runs (from the `runs` table): id, topic, status, turn count, total cost, timestamps.
- `GET /api/runs/{run_id}` — run metadata + final result (if complete).
- `GET /api/runs/{run_id}/events?after_seq=` — paged historical events for replay/late-join.
- `WS /api/runs/{run_id}/stream` — live event stream. On connect, replays existing events for that run (so a late joiner catches up), then pushes new events as they occur, ending with `sim.completed`/`sim.failed`.
...
- A list of past runs (`GET /api/runs`); selecting a completed run **replays** its event stream into the same cast-board/feed UI from stored events (`GET /api/runs/{id}/events`), so a completed run looks the same as a live one. Read-only.

### 10. Packaging (extend Phase 0, don't fork it)
