...
- **Memorable name** — a short, memorable, **unique** two-word kebab-case codename that is **thematically resonant with the topic** (e.g. topic "AI ethics & responsibility" → `trusted-robot`; "weekend hiking trip" → `summit-compass`; "AI in creative work" → `muse-machine`). Generated by a **single cheap LLM call at run creation** (see naming spec below). **Editable** by the user in the new-run form, with a **re-roll button** to regenerate.
- **Description** — a one-line human description (≤12 words), produced by the same call, editable, shown in the history list and dossier header.
...
#### 10a. Codename generation spec
- **Mechanism:** one short `litellm.acompletion` call using the run's configured model (or a cheap default), prompted to return strict JSON `{"name": "<two-word-kebab>", "description": "<≤12 words>"}`.
- **Prompt intent:** "Invent a memorable two-word codename (lowercase, hyphenated, e.g. `trusted-robot`) that evokes the theme of this simulation topic, plus a one-line description." Include the topic and cast names for flavor.
- **Constraints on the name:** exactly two words, `^[a-z]+-[a-z]+$`, no profanity/PII, not generic (`test-run`, `new-sim`). Validate the model output against the regex; if it fails, retry once, then fall back.
- **Uniqueness:** check against existing `runs.name`. On collision, ask the model for an alternative (or append a short disambiguating suffix like `-2`). Never block run creation on naming.
- **Fallback (must not fail a run):** if the LLM call errors or returns garbage, generate a random `adjective-noun` from a small built-in wordlist and a description derived from the topic string. Naming is convenience, never a hard dependency — same principle as avatars.
- **Cost:** one tiny call at startup; include its tokens/cost in the run's cost accounting (or note it as negligible). Do not fabricate its cost.
...
- [ ] **Named runs:** a new run gets a memorable, unique, **topically-resonant** two-word codename (LLM-generated, e.g. an AI-ethics topic yields something like `trusted-robot`) and description; naming falls back to a random wordlist name if the LLM call fails (run still created); the run is reloadable in the UI by that name via `GET /api/runs/{name}`, and the history list is filterable by name.
